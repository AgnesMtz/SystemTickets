// import {
//   Card,
//   CardHeader,
//   CardBody,
//   CardFooter,
//   Typography,
// } from "@material-tailwind/react";
// import PropTypes from "prop-types";
// import axios from 'axios';



// // StatisticsChart.displayName = "${import.meta.env.VITE_BACKEND_URL}/api/usuarios/registrar-cliente/";

// //  //Entra solo una vez para comprobar que el usuario exista
// //  useEffect(() => {
// //   const buscarCliente = async () => {
// //     try {
// //         const {data} = await axios.get(
// //           `${import.meta.env.VITE_BACKEND_URL}/api/usuarios/obtener-cliente/${params.id}`
// //           );
           
// //       } catch (error) {
// //         console.log(error);
// //       }
// //     };
// // }, []);  

// export function StatisticsChart({ color, chart, title, description, footer }) {
//   return (
//     <Card>
//       <CardHeader variant="gradient" color={color}>
//         <Chart {...chart} />
//       </CardHeader>
//       <CardBody className="p-6">
//         <Typography variant="h6" color="blue-gray">
//           {title}
//         </Typography>
//         <Typography variant="small" className="font-normal text-blue-gray-600">
//           {description}
//         </Typography>
//       </CardBody>
//       {footer && (
//         <CardFooter className="border-t border-blue-gray-50 px-6 py-5">
//           {footer}
//         </CardFooter>
//       )}
//     </Card>
//   );
// }



// StatisticsChart.defaultProps = {
//   color: "blue",
//   footer: null,
// };

// StatisticsChart.propTypes = {
//   color: PropTypes.oneOf([
//     "white",
//     "blue-gray",
//     "gray",
//     "brown",
//     "deep-orange",
//     "orange",
//     "amber",
//     "yellow",
//     "lime",
//     "light-green",
//     "green",
//     "teal",
//     "cyan",
//     "light-blue",
//     "blue",
//     "indigo",
//     "deep-purple",
//     "purple",
//     "pink",
//     "red",
//   ]),
//   chart: PropTypes.object.isRequired,
//   title: PropTypes.node.isRequired,
//   description: PropTypes.node.isRequired,
//   footer: PropTypes.node,
// };
// //ruta al backend


// StatisticsChart.displayName = "/src/widgets/charts/statistics-chart.jsx";

// export default StatisticsChart;

import React, { useEffect, useState } from 'react';
import axios from 'axios';
import {
  Card,
  CardHeader,
  CardBody,
  CardFooter,
  Typography,
} from "@material-tailwind/react";
import PropTypes from "prop-types";
import Chart from "react-apexcharts";

export function StatisticsChart({ color, chart, title, description, footer }) {
  const [numTicketsTerminados, setNumTicketsTerminados] = useState(null);

  const numeroTicketsTerminados = async () => {
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/estadisticas/obtener-ticketsTerminados/`
      );
       // console.log(data)
      // Actualiza el estado con los datos obtenidos del backend
      setNumTicketsTerminados(data.num_tickets);

    } catch (error) {
      console.log(error);
    }
  };

  const numeroTicketsTerminadosDos = async () => {
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/estadisticas/obtener-ticketsTerminados/`
      );
        console.log(data)
      // Actualiza el estado con los datos obtenidos del backend
      setNumTicketsTerminados(data.num_tickets);

    } catch (error) {
      console.log(error);
    }
  };


  useEffect(() => {
    // Llama a la función para obtener el número de tickets terminados
    numeroTicketsTerminados();
  }, []);  // El segundo argumento del useEffect es un array de dependencias, en este caso, está vacío para que se ejecute solo una vez al montar el componente

  return (
    <Card>
      <CardHeader variant="gradient" color={color}>
        <Chart {...chart} />
      </CardHeader>
      <CardBody className="p-6">
        <Typography variant="h6" color="blue-gray">
          {title}
        </Typography>
        <Typography variant="small" className="font-normal text-blue-gray-600">
          {description}
        </Typography>
      </CardBody>
      {footer && (
        <CardFooter className="border-t border-blue-gray-50 px-6 py-5">
          {footer}
        </CardFooter>
      )}
     
    </Card>
  );
}

StatisticsChart.defaultProps = {
  color: "blue",
  footer: null,
};

StatisticsChart.propTypes = {
  color: PropTypes.oneOf([
    "white",
    "blue-gray",
    "gray",
    "brown",
    "deep-orange",
    "orange",
    "amber",
    "yellow",
    "lime",
    "light-green",
    "green",
    "teal",
    "cyan",
    "light-blue",
    "blue",
    "indigo",
    "deep-purple",
    "purple",
    "pink",
    "red",
  ]),
  chart: PropTypes.object.isRequired,
  title: PropTypes.node.isRequired,
  description: PropTypes.node.isRequired,
  footer: PropTypes.node,
};

StatisticsChart.displayName = "/src/widgets/charts/statistics-chart.jsx";

export default StatisticsChart;
